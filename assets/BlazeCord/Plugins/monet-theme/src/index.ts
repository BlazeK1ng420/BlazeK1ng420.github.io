...
export const patchesURL = () =>
	`https://raw.githubusercontent.com/nexpid/RelapseMonetTheme/${
...
export const getSysColors = () =>
	(window as any).__relapse_syscolors as RelapseSysColors | null;

export const hasThemeKey = Symbol.for("monettheme.isThemed");
export const hasTheme = () => {
	const { blaze, [hasThemeKey]: isThemed } = window as any;
	return (
		!blaze.themes.getCurrentTheme()
		&& (blaze.themes.getThemeFromLoader()?.id === "monet-theme" || isThemed)
	);
};
