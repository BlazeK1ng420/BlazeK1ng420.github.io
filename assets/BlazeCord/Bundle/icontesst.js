...
${log.errorStack}`;
  ...
  function useHighlight() {
    var context17 = (0, import_react13.useContext)(HighlightContext);
    if (!context17) {
  ...
  var init_toasts3 = __esm({
    "src/plugins/_api/toasts/index.tsx"() {
      ...
      toasts_default = definePlugin({
        name: "Toast API",
        description: "BlazeCord toast notification API.",
        authors: [
          Devs.Blaze
        ],
        required: true,
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "dcaa0a00ddefcc6b5db65c76d135b5a86f37a868";
      branch = "emojis";
...
(${discord.build})`,
  ...
  var init_settings3 = __esm({
    "src/plugins/_api/settings/index.tsx"() {
      ...
      settings_default = definePlugin2({
        name: "Settings",
        description: "Adds custom settings ui and debug info to BlazeCord.",
        authors: [
          Devs.Blaze
        ],
        required: true,
  ...
  var init_dummy2 = __esm({
    "src/plugins/dummy/index.ts"() {
      ...
      dummy_default = definePlugin3({
        name: "Dummy",
        description: "It's a dummy, just like you~",
        authors: [
          Devs.Blaze
        ],
  ...
  var init_expression_utils2 = __esm({
    "src/plugins/expression-utils/index.tsx"() {
      ...
      expression_utils_default = definePlugin4({
        ...
        description: "Adds ui to clone & upload, or to copy CDN links, for Emojis & Stickers.",
        authors: [
          Devs.Blaze
        ],
        patches: [
  ...
  // plugins-context-provider:#plugin-context#src/plugins/nitro-emoji/index.ts
  ...
  var init_nitroChecks = __esm({
    "src/plugins/nitro-emoji/patches/nitroChecks.ts"() {
      ...
      nitroChecks_default = [
        {
          id: "unlock-nitro-checks",
          target: byProps([
            "canUseEmojisEverywhere",
            "canUseAnimatedEmojis"
          ]),
          patch(module3, patcher17) {
            patcher17.instead(module3, "canUseEmojisEverywhere", () => true);
            patcher17.instead(module3, "canUseAnimatedEmojis", () => true);
          }
        }
      ];
    }
  ...
  function extractEmojis(message, size, hyperlink, guildId, force) {
    var matches = [
      ...message.matchAll(/<a?:(\w+):(\d+)>/g)
    ];
    for (var match of matches) {
      var [, name, id] = match;
      var emoji = getCustomEmojiById?.(id);
      if (!emoji) continue;
      if (!force && emoji.guildId === guildId) continue;
      var ext = emoji.animated ? "gif" : "webp";
      var url = `https://cdn.discordapp.com/emojis/${id}.${ext}?size=${size}&quality=lossless&name=${name}`;
      message = message.replace(match[0], hyperlink ? `[${name}](${url})` : url);
    }
    return message.trim();
  }
  function replaceTextEmoji(message, size, hyperlink, guildId, force) {
    var emojiContext = getDisambiguatedEmojiContext?.();
    var map = emojiContext?.emojiContext?.emojiNameToID ?? {};
    return message.replace(/:([\w-]+):/g, (full, name) => {
      var ids = map[name];
      if (!Array.isArray(ids)) return full;
      var id = ids.find((id2) => {
        var emoji2 = getCustomEmojiById?.(id2);
        if (!emoji2) return false;
        return force || emoji2.guildId !== guildId;
      });
      if (!id) return full;
      var emoji = getCustomEmojiById?.(id);
      var ext = emoji?.animated ? "gif" : "webp";
  ...
  function modifyIfNeeded(msg) {
    console.log("[freemoji] \u26A0 modifyIfNeeded called");
    if (!msg || !msg.content) {
      console.log("[freemoji] \u{1F6AB} No message or message content");
      return;
    }
    console.log("[freemoji] \u{1F538} Original msg.content:", msg.content);
    var { emojiSize, hyperlink, forceMoji } = settings3.get();
    console.log("[freemoji] \u{1F527} Settings:", {
      emojiSize,
      hyperlink,
      forceMoji
    });
    console.log("[freemoji] \u{1F7E2} haveNitro =", globalThis.haveNitro);
    if (!forceMoji && globalThis.haveNitro) {
      console.log("[freemoji] \u26D4 Skipping because user has Nitro and forceMoji is false");
      return;
    }
    var guildId = getGuildId?.();
    console.log("[freemoji] \u{1F3F7} Current guild ID:", guildId);
    var regexMatch = msg.content.match(/<a?:(\w+):(\d+)>/g);
    if (!regexMatch) {
      console.log("[freemoji] \u274C No emoji match found in msg.content");
    } else {
      console.log("[freemoji] \u2705 Emoji matches found:", regexMatch);
    }
    var newContent = extractEmojis(msg.content, emojiSize, hyperlink, guildId, forceMoji);
    console.log("[freemoji] \u270F After extractEmojis:", newContent);
    newContent = replaceTextEmoji(newContent, emojiSize, hyperlink, guildId, forceMoji);
    console.log("[freemoji] \u270F After replaceTextEmoji:", newContent);
    msg.content = newContent;
    msg.invalidEmojis = [];
    var emojiMatches = [
      ...msg.content.matchAll(/<a?:(\w+):(\d+)>/g)
    ];
    console.log("[freemoji] \u{1F9E9} Final emojiMatches:", emojiMatches);
    if (emojiMatches.length > 0 && !msg.validNonShortcutEmojis?.length) {
      msg.validNonShortcutEmojis = emojiMatches.map(([full, name, id]) => {
        var emoji = getCustomEmojiById?.(id);
        var animated = full.startsWith("<a:");
        return {
          name,
          id,
          animated,
      ...
      console.log("[freemoji] \u{1F9E9} Injected fallback validNonShortcutEmojis:", msg.validNonShortcutEmojis);
    }
    console.log("[freemoji] \u2705 Final modified msg object:", JSON.stringify(msg, null, 2));
  }
  var emojiMod, guildMod, getCustomEmojiById, getDisambiguatedEmojiContext, getGuildId;
  var init_msgProcessor = __esm({
    "src/plugins/nitro-emoji/msgProcessor.ts"() {
      ...
      getCustomEmojiById = emojiMod.getCustomEmojiById;
      getDisambiguatedEmojiContext = emojiMod.getDisambiguatedEmojiContext;
      getGuildId = guildMod.getGuildId;
      __name(extractEmojis, "extractEmojis");
      __name(replaceTextEmoji, "replaceTextEmoji");
      __name(modifyIfNeeded, "modifyIfNeeded");
    }
  });

  // src/plugins/nitro-emoji/patches/sendMessage.ts
  var msgMod, uploadMod, sendMessage_default;
  var init_sendMessage = __esm({
    "src/plugins/nitro-emoji/patches/sendMessage.ts"() {
      ...
      sendMessage_default = [
        {
          id: "patch-send-message",
          target: msgMod,
          patch(module3, patcher17) {
            patcher17.before(module3, "sendMessage", (args) => {
              console.log("[freemoji] Before sendMessage:");
              console.log("args[1] =", args[1]);
              modifyIfNeeded(args[1]);
              console.log("[freemoji] After modifyIfNeeded:");
              console.log("args[1] =", args[1]);
            });
          }
        },
        ...uploadMod?.uploadLocalFiles ? [
          {
            id: "patch-upload-local-files",
            target: uploadMod,
            patch(module3, patcher17) {
              patcher17.before(module3, "uploadLocalFiles", (args) => {
                console.log("[freemoji] Before uploadLocalFiles:");
                console.log("args[0]?.parsedMessage =", args[0]?.parsedMessage);
  ...
  var init_nitro_emoji2 = __esm({
    "src/plugins/nitro-emoji/index.ts"() {
      ...
      init_nitroChecks();
      init_sendMessage();
      settings3 = definePluginSettings6({
        emojiSize: {
          type: "slider",
          label: "Emoji Size",
          points: [
        ...
        hyperlink: {
          ...
          description: "Links emoji to their CDN URL to avoid Discord restrictions.",
          default: true
        },
        forceMoji: {
          type: "boolean",
          label: "Force Freemoji",
          description: "Apply even if user has Nitro (useful for testing).",
          default: false
        }
      });
      haveNitro = false;
      nitro_emoji_default = definePlugin6({
        name: "Nitro Emoji Unlocker",
        description: "Use any emoji, anywhere.",
        authors: [
          Devs.Blaze
        ],
        patches: [
        ...
        start() {
          waitFor(byStoreName("UserStore"), (UserStore2) => {
            haveNitro = UserStore2.getCurrentUser()?.premiumType != null;
          });
          showToast("\u{1F438} Freemoji Enabled");
        },
        cleanup() {
          showToast("\u{1F614} Freemoji Disabled");
  ...
  var init_devtoolkit2 = __esm({
    "src/plugins/devtoolkit/index.ts"() {
      ...
      devtoolkit_default = definePlugin7({
        name: "DevToolkit",
        description: "Connects to a debugger webhook at localhost:9090.",
        authors: [
          Devs.Blaze
        ],
        version: "1.0.0",
...
${inspect({
        ...
        patches: [
          {
            ...
            patch(module3, patcher17) {
              ...
              patcher17.instead(module3, "getIcons", () => mainIcons);
              patcher17.instead(module3, "getOfficialAlternateIcons", () => altIcons);
              patcher17.after(module3, "getIconById", (_args, ret) => {
                ret.isPremium = false;
                return ret;
              });
            }
  ...
  var init_experiments2 = __esm({
    "src/plugins/experiments/index.ts"() {
      ...
      experiments_default = definePlugin9({
        name: "Experiments",
        description: "Enables overriding Discord experiments.",
        authors: [
          Devs.Blaze
        ],
        patches: [
      ...
      chat_bubbles_default = definePlugin10({
        ...
        start() {
          ...
          var updateBubbleAppearance = /* @__PURE__ */ __name(() => {
            ...
            BubbleModule_default.configure(avatarRadius, bubbleChatRadius, color).catch(logger14.error);
  ...
  var init_no_track2 = __esm({
    "src/plugins/_core/no-track/index.ts"() {
      ...
      no_track_default = definePlugin12({
        name: "NoTrack",
        description: "Kills Discord's internal tracking and crash reporting",
        authors: [
          Devs.Blaze
        ],
        required: true,
  ...
  function patchDefinitionAndResolver() {
    ...
    patcher14.before(isThemeModule, "isThemeDark", callback);
    patcher14.before(isThemeModule, "isThemeLight", callback);
    patcher14.before(getNativeModule("NativeThemeModule"), "updateTheme", callback);
    patcher14.instead(tokensModule.default.internal, "resolveSemanticColor", (args, orig) => {
  ...
  function patchSelectivelySyncedUserSettingsStore() {
    ...
    patcher15.after(SelectivelySyncedUserSettingsStore, "getState", (_, ret) => {
    ...
    patcher15.instead(SelectivelySyncedUserSettingsStore, "persist", (_, orig) => {
      isPersisting = true;
      try {
        orig();
      } finally {
  ...
  var init_painter2 = __esm({
    "src/plugins/_core/painter/index.ts"() {
      ...
      init_resolver2();
      init_stores3();
      painter_default = definePlugin13({
        name: "Painter",
  ...
  function startPlugin(draft, id) {
    ...
    if (isSafeModeEnabled() && !plugin.required) {
      logger20.info(`Plugin ${plugin.$id} is not required and safe mode is enabled, skipping`);
      return;
    }
    if (plugin.isAvailable?.() === false) {
      logger20.warn(`Plugin ${plugin.$id} is not available, skipping`);
      return;
    }
    if (draft.states[id].running) {
      logger20.warn(`${plugin.$id} already started`);
      return;
    }
    logger20.debug(`Starting plugin '${plugin.$id}'`);
    try {
      applyPluginPatches(id, plugin);
      draft.states[id].running = true;
      plugin.start?.();
    } catch (e3) {
      logger20.error`Failed to start ${plugin.$id}: ${e3}`;
      return;
    }
    return;
  }
  function applyPluginPatches(id, plugin) {
    var _loop = /* @__PURE__ */ __name(function(pluginPatch2) {
      ...
      var apply = /* @__PURE__ */ __name(() => {
        logger20.debug(`Applying ${patcher17.id} patch`);
        patcher17.reuse();
      ...
      if (settings5 && pluginPatch2.predicate) {
        ...
        var unsub = settings5.subscribe(() => predicate(), () => {
          if (predicate()) {
            ...
            logger20.debug(`Disposing ${patcher17.id} patch`);
            patcher17.dispose();
          }
        }, {
  ...
  function cleanupPlugin(draft, id) {
    var plugin = PLUGINS[id];
    if (plugin.required) {
      logger20.warn(`Cannot stop required plugin '${plugin.$id}'`);
      if (true) throw new Error(`Cannot stop required plugin '${plugin.$id}'`);
      return;
    }
    if (!draft.states[id].running) {
      logger20.warn(`${plugin.$id} already stopped`);
      return;
    }
    logger20.info(`Cleaning up plugin ${plugin.$id}`);
    try {
      ...
      logger20.error(`Failed to cleanup ${plugin.$id}: ${e3}`);
      return;
    }
  ...
  var logger20, PLUGINS, usePluginStore, usePluginStore_default;
  var init_usePluginStore = __esm({
    "src/stores/usePluginStore.ts"() {
      ...
      init_modules();
      init_utils2();
      logger20 = wtlogger.createChild("PluginStore");
  ...
  function parseColorManifest(manifest) {
    ...
    for (var [semanticColorKey, semanticColorValue] of Object.entries(manifest.main.colors.semantic ?? {})) {
      ...
      if (value[0] === "#") {
        ...
        if (!rawColorValue) {
          logger21.warn(`Unknown raw color reference: ${value}`);
          continue;
        }
        semanticColorDefinitions[semanticColorKey] = {
          value: rawColorValue,
  ...
  var logger22, formDividerModule, tokenRefModule, UserSettingsActionCreators, _inc, useThemeStore;
  var init_useThemeStore = __esm({
    "src/plugins/_core/painter/useThemeStore.ts"() {
      ...
      init_solar_bloom();
      init_aurora_violet();
      logger22 = wtlogger.createChild("useThemeStore");
