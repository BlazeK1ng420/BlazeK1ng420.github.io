...
(() => {
    ...
    var handlePickImage = /* @__PURE__ */ __name(() => _async_to_generator(function* () {
      try {
        if (DocumentPicker?.pickSingle) {
          ...
          var files = yield DocumentsNew.pick({
          ...
          if (firstFile?.uri) {
            ...
            var keptCopies = yield DocumentsNew.keepLocalCopy({
              files: [
                {
                  fileName: name2,
                  uri: firstFile.uri
        ...
        if (DocumentPicker?.isCancel && DocumentPicker.isCancel(e3)) {
        } else {
          console.error(e3);
        }
      }
  ...
  var import_react7, import_react_native13, DocumentPicker, DocumentsNew, styles2;
  var init_AddWallpaperSheet = __esm({
    "src/components/Blaze/Settings/pages/Wallpapers/AddWallpaperSheet.tsx"() {
      ...
      init_wrappers();
      init_wallpaperStore();
      DocumentPicker = lookupByProps("pickSingle", "isCancel");
      DocumentsNew = lookupByProps("pick", "saveDocuments");
...
${log.errorStack}`;
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "ebb4bfb163687f1564000780a8329a03cf293483";
      branch = "nitro";
...
(${discord.build})`,
              ...
              patcher22.after(module3, "default", (_, ret) => {
                ...
                if (!settings.get().onTop) {
                  try {
                    ...
                    logger7.warn`Failed to insert settings sections next to account section: ${e3}`;
                  }
                }
                props.sections = [
                  ..._registeredSettingSections,
  ...
  function modifyIfNeeded(msg, haveNitro2, settings6) {
    if (!msg?.content?.match(/<a?:(\w+):(\d+)>/)) return;
    var { emojiSize, forceMoji, hyperlink } = settings6.get();
  ...
  var settings4, haveNitro, nitro_emojis_default;
  var init_nitro_emojis2 = __esm({
    "src/plugins/nitro-emojis/index.ts"() {
      ...
      init_parse();
      init_realshit();
      settings4 = definePluginSettings6({
        emojiSize: {
          type: "slider",
          label: "Display Size",
          points: [
          ...
          },
          {
            ...
            patch(module3, patcher22) {
              patcher22.before(module3, "sendMessage", ([, msg]) => {
                ...
                modifyIfNeeded(msg, haveNitro, settings4);
              });
            }
          },
          {
            ...
            patch(module3, patcher22) {
              patcher22.before(module3, "uploadLocalFiles", ([uploadArgs]) => {
                haveNitro = getStore("UserStore").getCurrentUser()?.premiumType != null;
                modifyIfNeeded(uploadArgs?.parsedMessage, haveNitro, settings4);
              });
            }
          },
          {
            ...
            patch(module3, patcher22) {
              patchRealmoji(module3, patcher22, () => settings4.get().realshit);
            }
          }
        ],
        start() {
...
${inspect({
  ...
  var init_chat_bubbles2 = __esm({
    "src/plugins/chat-bubbles/index.ts"() {
      ...
      init_modules();
      init_shallow3();
      settings5 = definePluginSettings13({
        avatarRadius: {
          type: "slider",
          label: "Avatar Curve",
          points: [
  ...
  function applyPluginFluxInterceptors(id, plugin, pluginPatcherContext) {
    var _loop = /* @__PURE__ */ __name(function(eventName2, cb2) {
      ...
      pluginPatcherContext.attachDisposer(() => {
        unintercept();
        logger25.debug(`Intercepted from flux event '${eventName2}' for plugin '${id}'`);
      });
    }, "_loop");
