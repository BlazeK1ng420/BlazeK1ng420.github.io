...
${log.errorStack}`;
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "bd2e932857b98f64351fe7e8b475a43825252523";
      branch = "flux";
...
${inspect({
  ...
  function patchDefinitionAndResolver(tokensModule2) {
    ...
    patcher14.instead(tokensModule2.default.internal, "resolveSemanticColor", (args, orig) => {
    ...
    var _loop = /* @__PURE__ */ __name(function(eventName2, cb2) {
      var unintercept = interceptFluxEvent((event) => {
        try {
          ...
          logger20.error`${id}: Error while handling ${event.type}: ${e3}`;
        }
      });
      logger20.debug(`Intercepted flux event '${eventName2}' for plugin '${id}'`);
      pluginPatcherContext.attachDisposer(() => {
        unintercept();
        logger20.debug(`Intercepted from flux event '${eventName2}' for plugin '${id}'`);
      });
    }, "_loop");
  ...
  var init_usePluginStore = __esm({
    "src/stores/usePluginStore.ts"() {
      ...
      init_utils2();
      init_flux();
      logger20 = blzlogger.createChild("PluginStore");
  ...
  var init_useThemeStore = __esm({
    "src/plugins/_core/painter/useThemeStore.ts"() {
      ...
      init_solar_bloom();
      init_aurora_violet();
      logger22 = blzlogger.createChild("ThemeStore");
