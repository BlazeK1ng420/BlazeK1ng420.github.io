...
${log.errorStack}`;
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "2610939d52ba993c6e3aef8616c0370fc634b325";
      branch = "settings";
  ...
  var init_stealth = __esm({
    "src/plugins/_api/commands/coms/stealth.ts"() {
      ...
      stealth_default = /* @__PURE__ */ __name(() => ({
        ...
        execute(args, ctx) {
          var ephemeralArg = args.find((arg) => arg.name === "ephemeral");
          var ephemeral = ephemeralArg?.value ?? true;
          var current2 = usePluginStore_default.getState().settings.stealthTyping?.silentTypingActive ?? true;
          usePluginStore_default.getState().togglePlugin("stealthTyping", !current2, false);
          var status = !current2 ? "enabled" : "disabled";
          if (ephemeral) {
            messageUtil.sendBotMessage(ctx.channel.id, `Silent Typing is now **${status}**.`);
          } else {
            messageUtil.sendMessage(ctx.channel.id, {
              content: `Silent Typing is now **${status}**.`
            });
          }
        }
      }), "default");
