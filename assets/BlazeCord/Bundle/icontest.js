...
${log.errorStack}`;
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "a566c451705ee849ffe7cad01526a8ab4aeec7cc";
      branch = "emojis";
...
(${discord.build})`,
  ...
  var makeEmojiUrl, nitro_emoji_default;
  var init_nitro_emoji2 = __esm({
    "src/plugins/nitro-emoji/index.ts"() {
      ...
      init_toasts();
      init_constants();
      makeEmojiUrl = /* @__PURE__ */ __name((id, name, animated, size = 48) => `https://cdn.discordapp.com/emojis/${id}.${animated ? "gif" : "webp"}?size=${size}&quality=lossless&name=${name}`, "makeEmojiUrl");
      nitro_emoji_default = definePlugin6({
        name: "Real Freemoji",
        description: "Bypass Nitro emoji limits by replacing with image links.",
        authors: [
          Devs.Blaze
        ],
        start() {
          var MessageMod = byProps([
            "sendMessage",
            "receiveMessage"
          ]);
          ...
          if (!MessageMod.sendMessage || !EmojiMod.getCustomEmojiById || !GuildMod.getGuildId) {
            showToast("\u274C Freemoji failed to load modules");
            return;
          }
          var getEmoji = EmojiMod.getCustomEmojiById.bind(EmojiMod);
          var getGuildId = GuildMod.getGuildId.bind(GuildMod);
          var getEmojiCtx = EmojiMod.getDisambiguatedEmojiContext.bind(EmojiMod);
          instead(MessageMod, "sendMessage", (args, orig) => {
            var [channelId, msg] = args;
            var content = msg?.content;
            if (!content) return orig(...args);
            var currentGuild = getGuildId();
            var newContent = content;
            newContent = newContent.replace(/<a?:(\w+):(\d+)>/g, (_, name, id) => {
              var emoji = getEmoji(id);
              if (!emoji || emoji.guildId === currentGuild) return _;
              return makeEmojiUrl(id, name, emoji.animated);
            });
            var emojiContext = getEmojiCtx()?.emojiContext?.emojiNameToID ?? {};
            newContent = newContent.replace(/:([\w-]+):/g, (match, name) => {
              var ids = emojiContext[name];
              if (!ids?.length) return match;
              var emojiId = ids.find((id) => {
                var e4 = getEmoji(id);
                return e4 && e4.guildId !== currentGuild;
              });
              if (!emojiId) return match;
              var e3 = getEmoji(emojiId);
              return makeEmojiUrl(emojiId, name, e3.animated);
            });
            msg.content = newContent;
            msg.invalidEmojis = [];
            msg.validNonShortcutEmojis = [];
            return orig(channelId, msg);
          });
          showToast("\u{1F438} Freemoji Ready");
        },
        cleanup() {
          showToast("\u{1F614} Freemoji Unloaded");
        }
      });
    }
  });
...
${inspect({
  ...
  var init_chat_bubbles2 = __esm({
    "src/plugins/chat-bubbles/index.ts"() {
      ...
      init_modules();
      init_shallow3();
      settings3 = definePluginSettings10({
        avatarRadius: {
          type: "slider",
          label: "Avatar Curve",
          points: [
