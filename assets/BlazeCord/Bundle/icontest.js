...
${log.errorStack}`;
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "896abd8970477f3325041f217ba310781cedd96e";
      branch = "emojis";
...
(${discord.build})`,
  ...
  function modifyIfNeeded(msg) {
    ...
    var emojiMod = byProps([
      "getCustomEmojiById",
      "getDisambiguatedEmojiContext"
    ]);
    var guildMod = byProps([
      "getGuildId"
    ]);
    var getCustomEmojiById = emojiMod.getCustomEmojiById;
    var getGuildId = guildMod.getGuildId;
    var emojiContext = emojiMod.getDisambiguatedEmojiContext?.();
    var emojiNameToID = emojiContext?.emojiContext?.emojiNameToID ?? {};
    var currentGuild = getGuildId?.();
    msg.content = msg.content.replace(/<a?:(\w+):(\d+)>/gi, (match, name, id) => {
      var emoji = getCustomEmojiById?.(id);
      if (!emoji || emoji.guildId === currentGuild) return match;
      var ext = emoji.animated ? "gif" : "webp";
      var url = `https://cdn.discordapp.com/emojis/${id}.${ext}?size=${emojiSize}&quality=lossless&name=${name}`;
      return hyperlink ? `[${name}](${url})` : url;
    });
    msg.content = msg.content.replace(/:([\w-]+):/g, (match, name) => {
