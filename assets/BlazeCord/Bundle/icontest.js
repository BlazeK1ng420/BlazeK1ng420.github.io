...
${log.errorStack}`;
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "97a9d48f7e08ad017a8075a884a0658f70c2b881";
      branch = "emojis";
...
(${discord.build})`,
  ...
  function extractUnusableEmojis(messageString, size, hyperlink, currentGuild) {
    ...
    for (var emojiString of emojiStrings) {
      ...
      if (!emoji || emoji.guildId === currentGuild) continue;
      var ext = emoji.animated ? "gif" : "webp";
      var url = `https://cdn.discordapp.com/emojis/${emojiString[2]}.${ext}?size=${size}&quality=lossless&name=${emojiString[1]}`;
      messageString = messageString.replace(emojiString[0], hyperlink ? `[${emojiString[1]}](${url})` : url);
    }
    return {
  ...
  function modifyIfNeeded(msg) {
    console.log("[freemoji] Running modifyIfNeeded");
    console.log("Original content:", msg.content);
    if (!msg?.content?.match(hasEmotesRegex)) {
      console.log("[freemoji] No emoji regex match");
      return;
    }
    var { emojiSize, hyperlink, forceMoji } = settings3.get();
    if (!forceMoji && globalThis.haveNitro) {
      console.log("[freemoji] Skipping because haveNitro");
      return;
    }
    var emojiContext = getDisambiguatedEmojiContext?.();
    var emojiNameToID = emojiContext?.emojiContext?.emojiNameToID ?? {};
    var currentGuild = getGuildId?.();
    var { newContent } = extractUnusableEmojis(msg.content, emojiSize, hyperlink, currentGuild);
    var replacedPlain = newContent.replace(/:([\w-]+):/g, (match, name) => {
      var ids = emojiNameToID?.[name];
      if (!ids || !Array.isArray(ids)) return match;
      var emojiId = ids.find((id) => {
        var emoji2 = getCustomEmojiById?.(id);
        return emoji2 && emoji2.guildId !== currentGuild;
      });
      if (!emojiId) return match;
      var emoji = getCustomEmojiById?.(emojiId);
      if (!emoji) return match;
      var ext = emoji.animated ? "gif" : "webp";
      var url = `https://cdn.discordapp.com/emojis/${emojiId}.${ext}?size=${emojiSize}&quality=lossless&name=${name}`;
      return hyperlink ? `[${name}](${url})` : url;
    });
    console.log("Modified content:", replacedPlain);
    msg.content = replacedPlain;
    msg.invalidEmojis = [];
  }
  var emojiMod, guildMod, getCustomEmojiById, getDisambiguatedEmojiContext, getGuildId, hasEmotesRegex;
      ...
      sendMessage_default = [
        {
          ...
          target: msgMod,
          patch(module3, patcher17) {
            patcher17.instead(module3, "sendMessage", (args, orig) => {
        ...
        ...uploadMod?.uploadLocalFiles ? [
          {
            ...
            target: uploadMod,
            patch(module3, patcher17) {
              patcher17.instead(module3, "uploadLocalFiles", (args, orig) => {
                console.log("[freemoji] Before uploadLocalFiles:");
                console.log("parsedMessage =", args[1]?.parsedMessage);
                modifyIfNeeded(args[1]?.parsedMessage);
                console.log("[freemoji] After modifyIfNeeded:");
                console.log("parsedMessage =", args[1]?.parsedMessage);
                return orig(...args);
              });
