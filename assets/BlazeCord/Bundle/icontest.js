...
${log.errorStack}`;
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "ccc0e7d79b4d0b948046e74ad7cf1d49e09e2cad";
      branch = "emojis";
...
(${discord.build})`,
  ...
  function modifyIfNeeded(msg) {
    if (!msg?.content?.match(hasEmojiRegex)) return;
    var { emojiSize, hyperlink, forceMoji } = settings3.get();
    if (!forceMoji && globalThis.haveNitro) return;
  ...
  var emojiMod, guildMod, getCustomEmojiById, getDisambiguatedEmojiContext, getGuildId, hasEmojiRegex;
