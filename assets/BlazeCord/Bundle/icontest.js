...
${log.errorStack}`;
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "09e24ad30b6d53987d43692aa40bc8d3a232528a";
      branch = "emojis";
...
(${discord.build})`,
  ...
  function rewriteMessageContent(content, currentGuildId) {
    var { emojiSize, hyperlink } = settings3.get();
    var matches = content.matchAll(emojiRegex);
    var rewritten = content;
    for (var match of matches) {
      var [full, name, id] = match;
      var emoji = getCustomEmojiById?.(id);
      if (!emoji || emoji.guildId === currentGuildId) continue;
      var ext = emoji.animated ? "gif" : "webp";
      var url = `https://cdn.discordapp.com/emojis/${id}.${ext}?size=${emojiSize}&quality=lossless&name=${name}`;
      var replacement = hyperlink ? `[${name}](${url})` : url;
      rewritten = rewritten.replace(full, replacement);
    }
    return rewritten;
  }
  var emojiMod, guildMod, getCustomEmojiById, getDisambiguatedEmojiContext, getGuildId, emojiRegex, sendMessage_default;
  var init_sendMessage = __esm({
    "src/plugins/nitro-emoji/patches/sendMessage.ts"() {
      ...
      emojiRegex = /<a?:(\w+):(\d+)>/g;
      __name(rewriteMessageContent, "rewriteMessageContent");
      sendMessage_default = [
        {
          id: "freemoji-send-message",
          target: byProps([
            "sendMessage"
          ]),
          patch(module3, patcher17) {
            patcher17.before(module3, "sendMessage", (args) => {
              var msg = args[1];
              if (!msg?.content?.match(emojiRegex)) return;
              var { forceMoji } = settings3.get();
              var haveNitro2 = globalThis.haveNitro;
              if (!forceMoji && haveNitro2) return;
              var currentGuildId = getGuildId?.();
              if (!currentGuildId) return;
              msg.content = rewriteMessageContent(msg.content, currentGuildId);
              msg.invalidEmojis = [];
              if ("validNonShortcutEmojis" in msg) msg.validNonShortcutEmojis = [];
            });
          }
        }
      ];
    ...
    "src/plugins/nitro-emoji/index.ts"() {
      ...
      settings3 = definePluginSettings6({
        ...
        hyperlink: {
          ...
          description: "Wrap emoji in a clickable link to the CDN image.",
          default: true
        },
        forceMoji: {
          type: "boolean",
          label: "Force Even With Nitro",
          description: "Apply emoji rewriting even if you have Nitro.",
          default: false
        }
      });
      haveNitro = false;
      nitro_emoji_default = definePlugin6({
        name: "FreeEmoji",
        description: "Use any emoji by converting them into image links.",
        authors: [
          Devs.Blaze
        ],
        patches: [
        ...
        start() {
          ...
          showToast("\u{1F438} FreeEmoji enabled");
        },
        cleanup() {
          showToast("\u274C FreeEmoji disabled");
