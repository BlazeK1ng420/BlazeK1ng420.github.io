...
${log.errorStack}`;
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "f3143363785df65b085339b60cbcf4164bd47b3f";
      branch = "emojis";
...
(${discord.build})`,
  ...
  // src/plugins/nitro-emoji/parse.ts
  function modifyIfNeeded(msg, haveNitro2) {
    if (!msg?.content?.match(/<a?:(\w+):(\d+)>/)) return;
    var opts = settings3.get();
    var size = opts.emojiSize;
    if (!opts.forceMoji && haveNitro2) return;
    msg.content = msg.content.replace(/<(a?):(\w+):(\d+)>/gi, (match, animatedFlag, name, id) => {
      if (getGuildId && match.includes(getGuildId())) return match;
      var ext = animatedFlag === "a" ? "gif" : "webp";
      var url = `https://cdn.discordapp.com/emojis/${id}.${ext}?size=${size}&quality=lossless&name=${name}`;
      return opts.hyperlink ? `[${name}](${url})` : url;
    });
    msg.invalidEmojis = [];
  }
  var guildMod, getGuildId;
  var init_parse = __esm({
    "src/plugins/nitro-emoji/parse.ts"() {
      "use strict";
      init_wrappers();
      init_nitro_emoji2();
      guildMod = lookupByProps("getGuildId");
    ...
    "src/plugins/nitro-emoji/index.ts"() {
      ...
      settings3 = definePluginSettings6({
        ...
        hyperlink: {
          ...
          description: "Inject Emoji via hyperlink [Emoji](URL)",
          default: true
        },
        forceMoji: {
          type: "boolean",
          label: "Override Nitro",
          description: "Render emoji via plugin instead of paid Nitro. (Use Nitro)",
          default: false
        }
      });
      haveNitro = false;
      nitro_emoji_default = definePlugin6({
        ...
        patches: [
          {
            ...
            target: lookupByProps("canUseEmojisEverywhere", "canUseAnimatedEmojis"),
            patch(module3, patcher17) {
          ...
          },
          {
            ...
            target: lookupByProps("sendMessage", "receiveMessage"),
            patch(module3, patcher17) {
              patcher17.before(module3, "sendMessage", (args) => {
                modifyIfNeeded(args[1], haveNitro);
              });
            }
          },
          {
            id: "intercept-upload-local-files",
            target: lookupByProps("uploadLocalFiles"),
            patch(module3, patcher17) {
              if (!module3.uploadLocalFiles) return;
              patcher17.before(module3, "uploadLocalFiles", (args) => {
                modifyIfNeeded(args[0]?.parsedMessage, haveNitro);
              });
            }
          }
        ],
        start() {
          ...
          showToast("\u{1F525}NitroMoji's Unlocked\u{1F525}");
        },
        cleanup() {
          showToast("\u{1F6AB}NitroMoji's Locked\u{1F6AB}");
