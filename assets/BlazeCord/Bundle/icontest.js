...
${log.errorStack}`;
  ...
  var init_build_info = __esm({
    "build-info:#build-info"() {
      revision = "57bc1845d7b3b212a1c75ee398a2fade17673b51";
      branch = "emojis";
...
(${discord.build})`,
  ...
  function extractEmojis(message, size, hyperlink, guildId) {
    var matches = [
    ...
    for (var match of matches) {
      ...
      if (!emoji || emoji.guildId === guildId) continue;
  ...
  function replaceTextEmoji(message, size, hyperlink, guildId) {
    ...
    return message.replace(/:([\w-]+):/g, (full, name) => {
      ...
      var id = ids.find((id2) => {
        ...
        return emoji2 && emoji2.guildId !== guildId;
      });
      if (!id) return full;
      var emoji = getCustomEmojiById?.(id);
      var ext = emoji?.animated ? "gif" : "webp";
  ...
  function modifyIfNeeded(msg) {
    ...
    var newContent = extractEmojis(msg.content, emojiSize, hyperlink, guildId);
    newContent = replaceTextEmoji(newContent, emojiSize, hyperlink, guildId);
    msg.content = newContent;
    msg.invalidEmojis = [];
    if ("validNonShortcutEmojis" in msg) delete msg.validNonShortcutEmojis;
  }
